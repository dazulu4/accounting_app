...
class ApplicationConfig(BaseSettings):
    ...
    log_format: str = Field(
        default="json", pattern="^(json|text)$", description="Log format"
    )
...
class AWSConfig(BaseSettings):
    ...
    def validate_region(cls, v):
        ...
        if not v or not v.strip():
            raise ValueError(
                "AWS region cannot be empty. Please set AWS_REGION environment variable."
            )
        region = v.strip()
...
class AppSettings(BaseSettings):
    ...
    def validate_configuration(self):
        ...
        if (
            self.application.environment == EnvironmentEnum.PRODUCTION
            and self.application.debug
        ):
