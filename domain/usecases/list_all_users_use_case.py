...
class ListAllUsersUseCase:
    ...
    def execute(
        ...
        logger.info("list_all_users_completed", extra={"users_found": len(users)})
    ...
    def _retrieve_users(self, status_filter: Optional[UserStatusEnum]) -> List:
        ...
        if status_filter:
            ...
            logger.debug(
                ...
                extra={"status": status_filter.value, "count": len(users)},
            )
        else:
            users = self._user_gateway.find_all_users()
            logger.debug("all_users_retrieved", extra={"count": len(users)})
    ...
    def _calculate_user_statistics(self) -> dict:
        ...
        try:
            ...
            active_users = self._user_gateway.count_users_by_status(
                UserStatusEnum.ACTIVE
            )
            inactive_users = self._user_gateway.count_users_by_status(
                UserStatusEnum.INACTIVE
            )
            suspended_users = self._user_gateway.count_users_by_status(
            ...
            logger.warning(
                "user_statistics_calculation_failed", extra={"error": str(e)}
            )
    ...
    def _create_response(
        self, users: List, stats: dict, status_filter: Optional[UserStatusEnum]
    ) -> dict:
        """
            ...
            try:
                if hasattr(user, "to_dict"):
                    ...
                    users_data.append(
                        {
                            "user_id": getattr(
                                user, "user_id", getattr(user, "id", "unknown")
                            ),
                ...
                logger.warning(
                    ...
                    extra={"user": str(user), "error": str(e)},
                )
...
class GetUserStatsUseCase:
    ...
    def _calculate_comprehensive_statistics(self, users: List) -> dict:
        ...
        active_count = sum(
            1
            for user in users
            if getattr(user, "status", None) == UserStatusEnum.ACTIVE
        )
        inactive_count = sum(
            1
            for user in users
            if getattr(user, "status", None) == UserStatusEnum.INACTIVE
        )
        suspended_count = sum(
            1
            for user in users
            if getattr(user, "status", None) == UserStatusEnum.SUSPENDED
        )
