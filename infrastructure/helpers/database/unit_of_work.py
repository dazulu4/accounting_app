...
class UnitOfWork:
    ...
    def begin(self) -> None:
        ...
        try:
            ...
            logger.error("Failed to start transaction", extra={"error": str(e)})
            if self._session:
                self._session.close()
                self._session = None
    ...
    def commit(self) -> None:
        ...
        try:
            ...
            logger.error(
                ...
                extra={"error": str(e), "session_id": id(self._session)},
            )
    ...
    def close(self) -> None:
        ...
        if self._session:
            try:
                self._session.close()
                logger.debug("Session closed", extra={"session_id": id(self._session)})
            except Exception as e:
