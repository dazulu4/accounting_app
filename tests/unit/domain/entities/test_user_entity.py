...
class TestUserEntity:
    ...
    def test_is_active_logic(self):
        ...
        active_user = UserEntity(
            user_id=1, name="Active User", email="active@test.com", status=UserStatusEnum.ACTIVE
        )
        inactive_user = UserEntity(
            user_id=2, name="Inactive User", email="inactive@test.com", status=UserStatusEnum.INACTIVE
        )
        suspended_user = UserEntity(
            user_id=3, name="Suspended User", email="suspended@test.com", status=UserStatusEnum.SUSPENDED
        )
    ...
    def test_deactivate_and_activate_user(self):
        ...
        """
        user = UserEntity(
            user_id=1, name="Test User", email="test@test.com", status=UserStatusEnum.ACTIVE
        )
    ...
    def test_change_name_and_email(self):
        ...
        """
        user = UserEntity(
            user_id=1, name="Old Name", email="old@test.com", status=UserStatusEnum.ACTIVE
        )
    ...
    def test_change_name_and_email_with_invalid_data_raises_exception(self):
        ...
        user2 = UserEntity(
            user_id=2, name="Test User 2", email="test2@test.com", status=UserStatusEnum.ACTIVE
        )
