...
class TestHTTPErrorHandler:
    ...
    def test_handle_business_exception(self):
        ...
        with patch('infrastructure.helpers.errors.error_handlers.request', mock_request):
            response_data, status_code = HTTPErrorHandler.handle_exception(exception)
            
            assert status_code == 404
            assert response_data["error"]["type"] == "USER_NOT_FOUND"
            assert response_data["error"]["message"] == "[USER_NOT_FOUND] User with ID 'User not found' not found"
    ...
    def test_handle_validation_exception(self):
        ...
        with patch('infrastructure.helpers.errors.error_handlers.request', mock_request):
            ...
            assert response_data["error"]["message"] == "[VALIDATION_ERROR] Validation failed"
    ...
    def test_handle_internal_error_without_details(self):
        ...
        with patch('infrastructure.helpers.errors.error_handlers.request', mock_request):
            ...
            with patch.object(HTTPErrorHandler, '_should_include_details', return_value=False):
                response_data, status_code = HTTPErrorHandler.handle_exception(exception)
                
                assert status_code == 500
                assert response_data["error"]["type"] == "INTERNAL_ERROR"
                assert response_data["error"]["message"] == "An internal server error occurred. Please try again later."
    ...
    def test_handle_internal_error_with_details_in_development(self):
        ...
        with patch('infrastructure.helpers.errors.error_handlers.request', mock_request):
            ...
            with patch.object(HTTPErrorHandler, '_should_include_details', return_value=True):
                response_data, status_code = HTTPErrorHandler.handle_exception(exception)
...
class TestErrorResponseHelpers:
    ...
    def test_create_validation_error_response(self):
        ...
        with patch('infrastructure.helpers.errors.error_handlers.request', mock_request):
            field_errors = {"title": "Title is required", "description": "Description is required"}
